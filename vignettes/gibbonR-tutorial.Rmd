...
## Part 3b. Calculate performance metrics using 'ROCR'
```{r }
OutputDirectory <-  "/Users/denaclink/Desktop/RStudio Projects/gibbonR/data/DetectAndClassifyOutput"
  ...
  for(x in 1:nrow(ml.detection.df)){
    ...
    # if(all.detections.subset$class.label == '1'){
    #   
    #  temp.wav <- readWave(list.ml[[x]])
    #  wav.name <- paste('Test/',str_split_fixed(list.ml[[x]],pattern = '/',n=10)[,10],
    #                    sep='')
    # 
    #  writeWave(temp.wav,wav.name,extensible = F)
    #   
    # }
    
    all.detections.subset$trainingdata <- ListTrainingDirectories[k]
    #print(detections.ml.adj)
    recall.snr.all.df <- rbind.data.frame(recall.snr.all.df,all.detections.subset)
  }
  
  all.combinedprecision.recall.randomiter <- rbind.data.frame(all.combinedprecision.recall.randomiter,recall.snr.all.df)
  write.csv(all.combinedprecision.recall.randomiter,paste(range.secs.start,'secsrange_', 'randomiterforROCALLupdateF.csv',sep=''),row.names = F)
} 
```
