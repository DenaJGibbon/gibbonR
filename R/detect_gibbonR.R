...
detectGibbonR <- function(feature.df, model.type, tune = FALSE, wav.name, target.signal = "female.gibbon", which.quant = "intersection", min.freq = 0.4, max.freq = 2, n.windows = 9, density.plot = TRUE, 
    low.quant.val = 0.15, high.quant.val = 0.25, num.cep = 12, min.sound.event.dur = 4, max.sound.event.dur = 10, output = "wav", probability.thresh = 0.75, output.dir) {
    ...
    ## Neural network
    if (model.type == "NNET") {
        ml.model.nnet <- train(class ~ ., data = feature.df, method = "avNNet", trControl = trainControl(method = "CV", number = 5, classProbs = TRUE))
    }
    ...
    if (model.type == "SVM") {
        if (tune == TRUE) {
            ...
            tune.rad <- tune(svm, feature.df[, 2:ncol(feature.df)], feature.df$class, kernel = "radial", tunecontrol = tune.control(cross = 5), ranges = list(cost = c(0.001, 0.01, 0.1, 1, 2, 10, 100, 
                1000), gamma = c(0.01, 0.1, 0.5, 1, 2)))
            
            
            ml.model.svm <- svm(feature.df[, 2:ncol(feature.df)], feature.df$class, kernel = "radial", gamma = tune.rad$best.parameters$gamma, cost = tune.rad$best.parameters$cost, cross = 5, probability = TRUE)
            
        } else {
            ml.model.svm <- svm(feature.df[, 2:ncol(feature.df)], feature.df$class, kernel = "radial", gamma = 0.01, cost = 2, cross = 5, probability = TRUE)
        }
    }
    
    
    if (model.type == "GMM") {
        ml.model.gmm <- MclustDA(feature.df[, 2:ncol(feature.df)], class = feature.df$class, modelType = "EDDA")
    }
    ...
    N <- 1000
    combined.df <- data.frame(vals = c(rnorm(N, mean = fit$mu[1], sd = fit$sigma[1]), rnorm(N, mean = fit$mu[2], sd = fit$sigma[2])), labels = rep(c("Noise", "Signal"), each = N))
    ...
    # If user indicated maximum duration create list of sound events under certain duration if(max.sound.event.dur != 'NULL'){
    ...
    if (length(call.timing.list) == 0) {
        ...
        for (x in 1:length(call.timing.list)) {
            ...
            # Calculate MFCCs
            melfcc.output <- tuneR::melfcc(short.wav, minfreq = min.freq * 1000, hoptime = win.time, maxfreq = max.freq * 1000, numcep = num.cep, wintime = win.time)
            ...
            if (model.type == "NNET") {
                ...
                if (temp.nnet.df[1, ] >= probability.thresh) {
                  tuneR::writeWave(short.wav, filename = paste(output.dir, "/", target.signal, "_", model.type, "_", signal.probability, "_", swift.spectro$time[min(call.time.sub)], "_", swift.spectro$time[max(call.time.sub)], 
                    ".wav", sep = ""), extensible = F)
                  # 
