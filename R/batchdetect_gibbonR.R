...
batchDetectGibbonR <- function(input, feature.df, model.type, tune = FALSE, target.signal = "female.gibbon", which.quant = "intersection", min.freq = 0.4, max.freq = 2, n.windows = 9, density.plot = TRUE, 
    low.quant.val = 0.15, high.quant.val = 0.25, num.cep = 12, pattern.split = ".rda", min.sound.event.dur = 4, max.sound.event.dur = 10, output = "wav", probability.thresh = 0.75, wav.output = "TRUE", 
    output.dir = NULL) {
    ...
    ## Neural network
    if (model.type == "NNET") {
        ml.model.nnet <- caret::train(class ~ ., data = feature.df, method = "avNNet", trControl = caret::trainControl(method = "CV", number = 5, classProbs = TRUE))
    }
                ...
                for (x in 1:length(call.timing.list)) {
                  ...
                  if (model.type == "NNET") {
                    ...
                    if (temp.nnet.df[1, ] >= probability.thresh) {
                      if (wav.output == "TRUE") {
                        tuneR::writeWave(short.wav, filename = paste(output.dir, "/", temp.name, "_", target.signal, "_", model.type, "_", signal.probability, "_", swift.spectro$time[min(call.time.sub)], 
                          "_", swift.spectro$time[max(call.time.sub)], ".wav", sep = ""), extensible = F)
                      }
                      # 
