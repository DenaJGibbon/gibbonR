...
audioSegmentGMM <- function(wav.file, window.len = 512, window.type = "hanning", min.freq = 0.4, max.freq = 2, low.quant.val = 0.15, high.quant.val = 0.25, which.quant = "intersection", density.plot = TRUE,
    ...
    dens.plot <- ggplot2::ggplot(combined.df, ggplot2::aes(vals, fill = labels)) + ggplot2::geom_density(alpha = 0.45) + ggplot2::geom_vline(xintercept = max(vals.df[which.quant])) + ggplot2::scale_fill_manual(values = c("red",
        "blue")) + ggplot2::theme_bw() + ggplot2::xlab("Log-energy") + ggplot2::ylab("Density") + ggplot2::theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + ggplot2::guides(fill = guide_legend(title = "")) +
        ...
        if (output.type == "wav") {
            ...
            for (x in 1:length(call.timing.list)) {
                tryCatch({
                  ...
                  tuneR::writeWave(short.wav, filename = paste(output.dir, "/", "sound.event", "_", x, "_", swift.spectro$time[min(call.time.sub)], "_", swift.spectro$time[max(call.time.sub)], ".wav",
                    sep = ""), extensible = F)
        ...
        if (output.type == "mfcc") {
            for (x in 1:length(call.timing.list)) {
                tryCatch({
                  ...
                  # Calculate MFCCs
                  melfcc.output <- melfcc(short.wav, minfreq = min.freq * 1000, hoptime = win.time, maxfreq = max.freq * 1000, numcep = n.cep, wintime = win.time)

                  # Calculate delta cepstral coefficients
                  deltas.output <- deltas(melfcc.output)

                  # Also append .wav duration
                  mfcc.vector <- c(paste(from = swift.spectro$time[min(call.time.sub)], to = swift.spectro$time[max(call.time.sub)]), as.vector(t(melfcc.output[1:(n.window - 1), 2:n.cep])), as.vector(t(deltas.output[1:(n.window -
                    1), 2:n.cep])), wav.dur)
            ...
            sound.event.df <- matrix(unlist(sound.event.mfcc.list), nrow = length(sound.event.mfcc.list), ncol = length(sound.event.mfcc.list[[1]]), byrow = T)
