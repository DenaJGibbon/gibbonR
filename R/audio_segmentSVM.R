...
audioSegmentSVM <- function(wav.file, min.freq = 400, max.freq = 2000, win.hop.time = 0.25, n.cep = 12, tune = FALSE, cost.list = c(0.001, 0.01, 0.1, 1, 2, 10, 100, 1000), gamma.list = c(0.01, 0.1,
    0.5, 1, 2), cost.val = 1, gamma.val = 0.5, trainingdata, target.signal = "gibbon", prob.signal = 0.8, min.signal.dur = 1, writetodir = FALSE, output.dir) {
    ...
    melfcc.output <- tuneR::melfcc(temp.wav, minfreq = min.freq, maxfreq = max.freq, wintime = win.hop.time, hoptime = win.hop.time, numcep = n.cep)

    print("Calculating SVM")
    if (tune == "TRUE") {
        tune.rad.segmentation <- e1071::tune(svm, trainingdata[, 2:ncol(trainingdata)], trainingdata$class, type = "C-classification", kernel = "radial", tunecontrol = tune.control(cross = 5), ranges = list(cost = cost.list,
            gamma = gamma.list))
    ...
    svm.model <- e1071::svm(trainingdata[, 2:ncol(trainingdata)], trainingdata$class, kernel = "radial", type = "C-classification", gamma = gamma.val, cost = cost.val, cross = 5, probability = TRUE)
    ...
    if (nrow(call.timing.df) == 0) {
        ...
        for (b in 1:nrow(call.timing.df)) {
            ...
            if (writetodir == "TRUE") {
                tuneR::writeWave(short.wav, filename = paste(output.dir, "/", "sound.event", "_", b, "_", call.time.sub$starts, "_", call.time.sub$ends, ".wav", sep = ""), extensible = F)
            }
