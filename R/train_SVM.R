...
trainSVM <- function(feature.df, train.n = 0.7, test.n = 0.3, tune = "TRUE", cost = c(0.001, 0.01, 0.1, 1, 2, 10, 100, 1000), gamma = c(0.01, 0.1, 0.5, 1, 2), kernel = "radial", cross = 5) {
    ...
    if (tune == "TRUE") {
        tune.rad <- tune(svm, mfcc.train[, 2:ncol(mfcc.train)], mfcc.train$class, type = "C-classification", kernel = kernel, tunecontrol = tune.control(cross = 5), ranges = list(cost = cost, gamma = gamma))
        
        svm.model <- svm(mfcc.train[, 2:ncol(mfcc.train)], mfcc.train$class, kernel = kernel, type = "C-classification", gamma = tune.rad$best.parameters$gamma, cost = tune.rad$best.parameters$cost, 
            cross = cross)
    } else {
        svm.model <- svm(mfcc.train[, 2:ncol(mfcc.train)], type = "C-classification", mfcc.train$class, kernel = kernel, cross = cross)
    }
    mod.pred <- predict(svm.model, mfcc.test[, 2:ncol(mfcc.test)])
    
    confusion.mat <- table(pred = mod.pred, true = mfcc.test$class)
