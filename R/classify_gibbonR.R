...
classifyGibbonR <- function(feature.df, model.type, tune = FALSE, input.dir, plot = TRUE, min.freq = 400, max.freq = 2000, n.window = 9, n.cep = 12, min.sound.event.dur = 4, max.sound.event.dur = 10, 
    output = "wav", probability.thresh = 0.75, output.dir) {
    
    print("machine learning in progress...")
    ## Neural network
    if (model.type == "NNET") {
        ml.model.nnet <- caret::train(class ~ ., data = feature.df, method = "nnet", trControl = trainControl(method = "CV", number = 5))
    }
    ...
    if (model.type == "GMM") {
        ...
        tryCatch({
            ml.model.gmm <- mclust::MclustDA(feature.df[, 2:ncol(feature.df)], class = feature.df$class, modelType = "EDDA")
        }, error = function(e) {
            cat("ERROR :", conditionMessage(e), "\n")
        })
        
        if ((length(ml.model.gmm) == "NA")) 
    ...
    for (j in 1:length(input.dir.list.files)) {
        ...
        # Calculate MFCCs
        melfcc.output <- tuneR::melfcc(wav.file, minfreq = min.freq, hoptime = win.time, maxfreq = max.freq, numcep = n.cep, wintime = win.time)
        
        # Calculate delta cepstral coefficients
        deltas.output <- tuneR::deltas(melfcc.output)
        
        # Ensure only 8 time windows are used for MFCC and delta coefficients Also append .wav duration
        mfcc.vector <- c(as.vector(t(melfcc.output[1:(n.window - 1), 2:n.cep])), as.vector(t(deltas.output[1:(n.window - 1), 2:n.cep])), wav.dur)
    ...
    data.matrix.all <- matrix(unlist(mfcc.vector.list), nrow = length(unlist(file.name.list)), ncol = length(mfcc.vector.list[[1]]), byrow = T)
    
    
    class <- unlist(file.name.list)
    
    mfcc.data.frame <- cbind.data.frame(class, data.matrix.all)
