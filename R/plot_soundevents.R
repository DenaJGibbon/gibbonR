...
plotSoundevents <- function(input.dir, nrow=3, ncol=2, from=1, n.soundevents=length(list.wav.files),
                            window.size=1024, min.freq=400, max.freq=2500, class.label= "sound event",
                            label.wav.name=TRUE,
                             return.table=TRUE) {
...
for(a in from:n.soundevents){
  ...
  plot(temp.spec,xlab="Time (s)", ylab="Frequency (Hz)",col=viridis::viridis(512), ylim=c(min.freq,max.freq),
       main=paste(class.label,a),useRaster = TRUE)
